...
- **`map-config.js`** — constants only: default view, tile layers (OSM/Esri-Satellit/OpenTopo, all with `crossOrigin:'anonymous'` — required so export can draw tiles to a canvas), OSRM service URL, CDN URLs for export inlining, image size/quality, export filenames.
- **`map-core.js`** — `initMap()`, `setRouteWaypoints(map)` (feeds `state.pois` to the routing control; <2 POIs clears the route), `fitToMarkers(map)`. Routing events update `#routeinfo` ("Gesamt: X.X km").
- **`map-pois.js`** — `createPoi()` (shared factory for interactive creation AND import — always create POIs through it), `bindPoiPopup()`, `renumberAndRoute()`, `removePoi`/`removeLastPoi`/`clearPois`, `setupPOIs()` (creation gestures: desktop dblclick/right-click, mobile tap/long-press via `L.Browser.mobile`).
- **`map-ui.js`** — `openPoiDialog(map, p, 'create'|'edit')` (the one shared form: Bezeichnung, Link, image preview/pick/delete, Speichern, Abbrechen/Station löschen), toolbar wiring, lightbox (`openLightbox`), persistent file picker (`pickImage`).
- **`map-io.js`** — `setupIO()`, GeoJSON/GPX builders, imports (both funnel through `importFeatures` → `createPoi`), and the offline-capable single-file HTML export (`captureMapSnapshot` + `fetchLeafletAssets` + `buildSingleFileHtmlBlob`).
- **`map-utils.js`** — generic helpers: `readFileAsText`, `fileToDataURL` (HEIC conversion + 3-tier decode + downscale), `triggerBlobDownload` (with DuckDuckGo workaround), `escapeHtml`/`escapeXml`.
