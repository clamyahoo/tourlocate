...
- **`map-ui.js`** — `openPoiDialog(map, p, 'create'|'edit')` (the one shared form: name, link, link label, image preview/pick/delete, save, cancel/delete-station), sidebar wiring (collapsible `<aside id="sidebar">` with `<details>` sections; toggle state persists via the `sidebar` setting; `map.invalidateSize()` after toggling), sort/line-mode/profile/image-quality/language controls, WebDAV credential fields + import button, local photo import (button + drag-and-drop onto the map with a `#dropOverlay`, both via `runPhotoImport` → `importPhotoFiles`), fullscreen, help overlay, lightbox (`openLightbox`), persistent file picker (`pickImage`). `#routeinfo` is a floating pill over the map (also used as progress display during exports/imports).
- **`map-photos.js`** — `importPhotoFiles(map, files, onProgress)`: the local (no-server) counterpart to the WebDAV import — reads EXIF GPS/date from each dropped/picked file, places POIs, skips files without GPS, sorts by capture date, fits bounds. Shares the `createPoi`/`sortPois`/`fitToMarkers` path.
- **`map-exif.js`** — dependency-free EXIF reader (`readExif`: GPS + DateTimeOriginal for **JPEG and HEIC/HEIF**; HEIC path walks the ISOBMFF boxes `meta→iinf→iloc` to the `Exif` item, then parses its TIFF block; both byte orders — camera files are usually big-endian "MM") and JPEG writer (`writeExif`: builds a spec-compliant little-endian APP1 segment with GPS/date/description plus the EXIF-mandated base tags; heap values are word-aligned). Canvas re-encoding strips EXIF, so the ZIP export re-stamps images via `writeExif`.
- **`map-utils.js` HEIC conversion** — `fileToDataURL` converts HEIC/HEIF via `heicToJpegBlob`, which runs heic2any inside a **throwaway hidden iframe** (one per image, destroyed after). This is deliberate: heic2any shares a global libheif/WASM state that hangs on the *second* call in the same context (a batch import would freeze after the first photo), and it can't run in a Web Worker because it needs a `<canvas>`/`document`. Each iframe has its own document + fresh WASM instance. Do not "simplify" back to calling `heic2any()` directly in a loop.
- **`map-webdav.js`** — `importFromWebdav(map, onProgress)`: PROPFIND folder listing (namespace-aware DAV: parsing), GET per image, `readExif` for GPS/date, skips photos without GPS, creates POIs through `createPoi`, then sorts by capture date (`sortPois('date','asc')`) and fits bounds. All requests go through `webdav-proxy.php` (see below) via `proxyFetch()` instead of a direct cross-origin `fetch()` — `webdavErrorMessage` maps `'auth'`/`'proxy'`/`'nourl'` failures to translated hints.

**`webdav-proxy.php`** (repo root, next to `index.html`) — the **only server-side code** in an otherwise fully static project. Browsers cannot fetch a foreign WebDAV server directly (CORS), so this same-origin PHP script relays the PROPFIND/GET request server-side, where CORS doesn't apply. Credentials are passed through per-request from the browser's `localStorage` settings and never persisted server-side. **Deployment requires PHP + the `curl` extension** on the hosting; without it, WebDAV import is unavailable (rest of the app is unaffected). Hardening against abuse as an open relay — do not weaken without reintroducing equivalent protection:
  - Origin/Referer must match the request's own `Host` header, and a shared `key` (from `PROXY_KEY`, duplicated in `map-config.js`) must match — both are weak-but-cheap deterrents against blind scanners, not real auth (the key is visible in public JS source).
  - Target URL must be `https://` and only actions `list` (PROPFIND) / `get` (GET) are allowed; the resolved IP is checked with `filter_var(..., FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)` to block private/loopback/link-local targets (SSRF protection, also blocks the `169.254.169.254` cloud-metadata address) — this is the real security boundary, independent of the key.
  - `CURLOPT_FOLLOWLOCATION` is off (a redirect could otherwise bounce past the IP check post-validation); response size and timeout are capped.
  - Proxy-side failures set an `X-Proxy-Error` header **and put the concrete reason in the response body** (e.g. `curl-Fehler 60: SSL certificate problem`); `proxyFetch` surfaces that as `err.detail` so `webdavErrorMessage` can show the real cause. A non-OK response *without* that header means PHP didn't run at all (static host / missing file) → `'nophp'`. 401/403 without the header = the WebDAV server rejected auth → `'auth'`.
