...
- **`map-ui.js`** — `openPoiDialog(map, p, 'create'|'edit')` (the one shared form: name, link, image preview/pick/delete, save, cancel/delete-station), toolbar + settings row wiring (sort, line mode, profile, image quality, language toggle, fullscreen, help overlay), lightbox (`openLightbox`), persistent file picker (`pickImage`).
- **`map-io.js`** — `setupIO()`, GeoJSON builder (with `createdAt` property), Komoot-compatible GPX builder (`<metadata>`, `<time>` **before** `<name>` in `<wpt>` per GPX schema, route as `<trk>`), imports (both funnel through `importFeatures` → `createPoi`; GPX `<time>` restores `createdAt`), the offline-capable HTML export (`captureMapSnapshot` + `fetchLeafletAssets` + `buildExportHtml`) in two flavors: single file (images inline as base64) or ZIP with a `bilder/` folder (images as separate JPEGs, HTML references relative paths). Export filenames carry the date (`tourlocate-YYYY-MM-DD.*`).
