...
Es gibt keinen Build-Schritt, keinen Server-Unterbau und keine Abhängigkeiten außerhalb des Browsers — die Seite besteht aus reinem HTML/CSS/JavaScript (ES-Module) und lädt alle Bibliotheken über CDN.
...
- **WebDAV/Nextcloud-Import**: Ein WebDAV-Ordner (z. B. ein Nextcloud-Album) lässt sich als Bildquelle hinterlegen. Beim Import werden alle Fotos geladen, anhand ihrer GPS-Metadaten automatisch als Stationen platziert und nach Aufnahmedatum nummeriert — Fotos ohne Positionsdaten werden übersprungen.
...
Die Seite braucht einen einfachen HTTP-Server (ES-Module funktionieren nicht über `file://`):
...
| `map-webdav.js` | Import von Bildern aus einem WebDAV-/Nextcloud-Ordner |
