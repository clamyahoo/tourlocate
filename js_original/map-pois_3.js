...
export function addPoi(ctx, lat, lng, { title = '', desc = '', imgData = '' } = {}) {
  ...
  // Direkt Anzeige-Popup zeigen (wie früher nach Speichern)
  openViewPopup(marker);
  // Klick auf Marker öffnet Anzeige-Popup & Lightbox
...
// Cleanup evtl. vorherige Lightbox + neue initialisieren
tryCloseLightbox();
setTimeout(() => {
  const editBtn = document.querySelector('.leaflet-popup-content .edit-btn');
  if (editBtn) {
...
// Wenn das Leaflet-Popup geschlossen wird, Lightbox auch schließen
...
function closeEditPopup(saveChanges) {
  ...
  // evtl. offene Lightbox schließen
  tryCloseLightbox();
  editingMarker = null;
}
